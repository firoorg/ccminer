...
*/
	...
	ablake2b4rounds_update(&BlakeHash, inp, strlen(inp));
	ablake2b4rounds_final(&BlakeHash, hash, 16);
...
*/
	  ablake2b_state BlakeHash;
	  ablake2b_init(&BlakeHash, 16);
	  ablake2b4rounds_update(&BlakeHash, &tree[idx - 1], sizeof(uint256) / 2);
	  ablake2b4rounds_update(&BlakeHash, &tree[idx], sizeof(uint256) / 2);
	  ablake2b4rounds_final(&BlakeHash, (unsigned char*)&hash1, 16);

	  ablake2b_init(&BlakeHash, 16);
	  ablake2b4rounds_update(&BlakeHash, &hash1, sizeof(uint256) / 2);
	  ablake2b4rounds_final(&BlakeHash, (unsigned char*)&tree[parent], 16);
